...
//       method:"GET",//optimal:: default get
//       headers:{
...
// getdata();
  ...
  const showUI = (list) => {
    const fragment = document.createDocumentFragment(); // Batch UI updates
    ...
    root.innerHTML = ""; // Clear only when necessary
    root.appendChild(fragment); // Append all new elements at once
};

  
  // Function to handle hover effect on thumbnails
  const preloadedImages = dummyData.map(video => {
    const img = new Image();
    img.src = video.videoThumbnails[video.videoThumbnails.length - 1].url;
    return img;
});

const handleHover = (e, idx) => {
    e.target.src = preloadedImages[idx].src; // Use preloaded image instead of fetching again
};
  ...
  // Function to close video description
  ...
  showUI(dummyData); // Initial display of videos